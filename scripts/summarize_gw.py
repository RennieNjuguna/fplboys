...
def run(gw_num=4):
    ...
    # Flagged members: Benn Mwangi and King Chris
    flagged_names = ['Benn', 'Chris']

    print(f"\n================ GAMEWEEK {gw_num} COMPREHENSIVE OVERVIEW ================\n")
    for m in data['managers']:
        is_flagged = any(fn in m['manager_name'] for fn in flagged_names)
        status_str = "FLAGGED / UNPAID (DISQUALIFIED)" if is_flagged else "PAID ON TIME (ELIGIBLE)"
        ...
        print(f"   Financial Status: {status_str}")
        print("-" * 60)
